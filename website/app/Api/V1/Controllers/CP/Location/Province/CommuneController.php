...
class CommuneController extends ApiController
{
    ...
    function list($id = 0){
        ...
        $data = Main::select('id', 'name', 'code', 'district_id')
        ->withCount(['villages as n_of_villages'])
        ->with(
            array(
                'district'=>function($query){
                    $query->select('id', 'name', 'code', 'province_id')->orderBy('name', 'asc');
                }, 
                'district.province'=>function($query){
                    $query->select('id', 'name', 'en_name')->orderBy('name', 'asc');
                }
            )
        )
        ->whereHas('district', function ($query) use ($id) {
            $query->whereHas('province', function ($query) use ($id){
                $query->where('id', $id);
            });
        });
