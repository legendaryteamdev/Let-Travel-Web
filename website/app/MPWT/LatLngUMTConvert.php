...
class LatLngUMTConvert extends Controller
{
    
    public static function utm2ll( $x = 0, $y = 0, $zone = 48, $aboveEquator = true ){
        ...
        $latlng = Self::UTMXYToLatlng ($x, $y, $zone, $southhemi);
        return ['lat'=>Self::radian2degree($latlng[0]), 'lng'=>Self::radian2degree($latlng[1])];

    }

    public static function ll2utm( $lat = 0, $lng = 0){
        $lat = llformat($lat, 4); 
        $lng = llformat($lng, 3);

        if(!is_numeric($lng)){
            return json_encode(array('success'=>false,'msg'=>"Wrong lnggitude value"));
        }
        if($lng<-180.0 or $lng>=180.0){
            return json_encode(array('success'=>false,'msg'=>"The lnggitude is out of range"));
        }
        if(!is_numeric($lat)){
        ...
        $zone = floor(($lng + 180.0) / 6) + 1;
        //compute values
        $result = Self::LatlngToUTMXY(Self::degree2radian($lat), Self::degree2radian($lng),$zone);
        $aboveEquator = false;
        if($lat >0){
            $aboveEquator = true;
        }
    ...
    public static function radian2degree($rad){
        $pi = 3.14159265358979; 
            return ($rad / $pi * 180.0);
    }

    public static function degree2radian($deg){
        $pi = 3.14159265358979;
        return ($deg/180.0*$pi);
    }
    ...
    public static function LatlngToUTMXY ($lat, $lng, $zone = 48){
            $xy = Self::MapLatlngToXY ($lat, $lng, Self::UTMCentralMeridian($zone));
    ...
    public static function UTMXYToLatlng ($x, $y, $zone, $southhemi){
        $latlng = array();
        $UTMScaleFactor = 0.9996;
            $x -= 500000.0;
            $x /= $UTMScaleFactor;
            ...
            $latlng = Self::MapXYToLatlng ($x, $y, $cmeridian);   
            return $latlng;
    }

    public static function MapXYToLatlng ($x, $y, $lambda0){
        $philambda = array();
        $sm_b = 6356752.314;
        $sm_a = 6378137.0;
        $UTMScaleFactor = 0.9996;
    ...
    public static function FootpointLatitude ($y){
        ...
        $sm_EccSquared = .00669437999013;
            ...
            $epsilng_ = (1097.0 * pow ($n, 4.0) / 512.0);
    ...
    public static function MapLatlngToXY ($phi, $lambda, $lambda0){
        $xy=array();
        $sm_b = 6356752.314;
        $sm_a = 6378137.0;
        $UTMScaleFactor = 0.9996;
